...
public class Code03_CoinsWayNoLimit{
    ...
    public static int  process2(int[]arr, int index , int rest){
        ...
        int res = 0;
        for (int piece =0;rest-piece*arr[index]>=0;++piece){
            res += process2(arr, index+1, rest - piece * arr[index]);
        }
        return res;
    }
...
//            rest的起始位置!!!!
            for (int rest = 1;rest<=aim;++rest){
                for (int piece =0;rest-piece*arr[i]>=0;++piece) {
                    dp[i][rest] +=dp[i+1][rest-piece*arr[i]];
                }
            }
        }
        return dp[0][aim];
